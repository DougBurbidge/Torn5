		                                    ...
		                                    bool scaleGames, bool ratio, bool showColours, bool description)
		{
			...
			foreach (LeagueTeam team in league.Teams)  // Create a row for each League team.
			{
				...
				ZCell scoreCell;
		 		...
		 		{
		 			if ((from == null || league.Game(gameTeam).Time >= from) && (to == null || league.Game(gameTeam).Time <= to))
					{
		 				if (ratio)
		 				{
			...
			if (scaleGames && countList.Count > 0 && countList.Min() != countList.Max())  // Find the mode, and add scaled columns.
			{
...
//		    report.OnCalcBar = TeamLadderColourCalcBar;
		...
		public static ZoomReport TeamsVsTeams(League league, bool includeSecret, DateTime? from, DateTime? to, bool description, GameHyper gameHyper)
		{
			...
			foreach (var team1 in teams)
			{
				...
				{
					var games = league.Games(includeSecret).FindAll(x => x.Teams.Any(y => league.LeagueTeam(y) == team1) &&
					...
					{
						var wins = games.Count(x => x.Teams.FindIndex(y => league.LeagueTeam(y) == team1) < x.Teams.FindIndex(z => league.LeagueTeam(z) == team2));
...
//			report.MultiColumnOK = true;
			...
			report.Title = league.Title + " Games " + FromTo(games, from, to);
		...
		public static ZoomReport GamesGrid(League league, bool includeSecret, DateTime? from, DateTime? to, Drops drops, bool description, GridType gridType, GameHyper gameHyper)
		{
			...
			while (thisgame < games.Count && games[thisgame].Time < newFrom)
	 		...
	 		games = games.Where(g => g.Time >= newFrom && g.Time <= newTo).ToList();
		...
		public static ZoomReport GamesGridCondensed(League league, bool includeSecret, DateTime? from, DateTime? to, Drops drops, bool description, GridType gridType, GameHyper gameHyper)
		{
			...
			foreach (string title in titles)
			{
				...
				foreach (var team in league.Teams)
					maxCount = Math.Max(maxCount, games.FindAll(x => x.Title == title && x.Teams.Exists(y => league.LeagueTeam(y) == team)).Count);
				...
				if (games.Count() > 0)
				{
					...
					foreach (var played in player.Played)
					{
						...
						if (game != null)
						{
							...
							srxTrs.Add(Math.Sqrt(((double)played.Score) / game.TotalScore() * game.Players.Count * played.HitsBy / played.HitsOn));
						}
					}
					row.Add(DataCell(scoreRatios, drops, chartType, "P1"));  // Score ratio
					row.Add(DataCell(srxTrs, drops, chartType, "P1"));  // sqrt(SR x TR)

					row.Add(DataCell(player.Played.Select(x => (double)x.BaseHits).ToList(), drops, ChartType.Bar, "N1"));
					row.Add(DataCell(player.Played.Select(x => (double)x.BaseDestroys).ToList(), drops, ChartType.Bar, "N1"));
					row.Add(DataCell(player.Played.Select(x => (double)x.BaseDenies).ToList(), drops, ChartType.Bar, "N1"));
					row.Add(DataCell(player.Played.Select(x => (double)x.BaseDenied).ToList(), drops, ChartType.Bar, "N1"));
					row.Add(DataCell(player.Played.Select(x => (double)x.YellowCards).ToList(), drops, ChartType.Bar, "N1"));
					row.Add(DataCell(player.Played.Select(x => (double)x.RedCards).ToList(), drops, ChartType.Bar, "N1"));
		...
		public static void FillOneGameRow(ZRow row, GamePlayer gamePlayer, Color color, double averageScore)
		{
		...
		public static ZoomReport OneGame(League league, Game game)
		{
			...
			foreach (GameTeam gameTeam in game.Teams)
			{
				...
				foreach (GamePlayer gamePlayer in gameTeam.Players)
				{
					...
					team.HitsBy += gamePlayer.HitsBy;
					team.HitsOn += gamePlayer.HitsOn;
					team.BaseHits += gamePlayer.BaseHits;
					team.BaseDestroys += gamePlayer.BaseDestroys;
					team.BaseDenies += gamePlayer.BaseDenies;
					team.BaseDenied += gamePlayer.BaseDenied;
					team.YellowCards += gamePlayer.YellowCards;
					team.RedCards += gamePlayer.RedCards;
				...
				if (gameTeam.Adjustment != 0)
				{
					...
					FillOneGameRow(adjustmentRow, adjustment, color, 0);
				...
				team.Score = gameTeam.Score;
				FillOneGameRow(teamRow, team, teamColor, (double)game.TotalScore() / game.Players.Count * gameTeam.Players.Count);
			...
			gameRow.Add(new ZCell(totals.HitsBy / game.Teams.Count, ChartType.Bar, "N0"));
			gameRow.Add(new ZCell(totals.HitsOn / game.Teams.Count, ChartType.Bar, "N0"));

			if (totals.HitsOn == 0)  // Tag ratio
				gameRow.Add(new ZCell("infinite"));
			else
				gameRow.Add(new ZCell((double)totals.HitsBy / totals.HitsOn, ChartType.Bar, "P0"));  // Tag ratio

			gameRow.Add(new ZCell("100 %"));  // Score ratio
			...
			gameRow.Add(new ZCell(((double)totals.BaseDenies / game.Teams.Count), ChartType.None, "N1"));
			gameRow.Add(new ZCell(((double)totals.BaseDenied / game.Teams.Count), ChartType.None, "N1"));
			gameRow.Add(new ZCell(((double)totals.YellowCards / game.Teams.Count), ChartType.None, "N1"));
...
//			TODO: replace the above line with report.OnCalcBar = KombiReportCalcBar; TeamLadderCalcBar;
		...
		public static ZoomReport FixtureGrid(Fixture fixture, League league, int? teamId = null)
		{
			...
			ZoomReport report = new ZoomReport(title, "Teams", "left");
